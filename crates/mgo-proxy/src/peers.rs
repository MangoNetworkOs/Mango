...
/// MgoNods a mapping of public key to MgoPeer data
pub type MgoPeers = Arc<RwLock<HashMap<Ed25519PublicKey, MgoPeer>>>;

/// A MgoPeer is the collated mgo chain data we have about validators
...
/// MgoNodeProvider queries the mgo blockchain and keeps a record of known validators based on the response from
/// mgo_getValidators.  The node name, public key and other info is extracted from the chain and stored in this
...
impl MgoNodeProvider {
    ...
    pub fn get(&self, key: &Ed25519PublicKey) -> Option<MgoPeer> {
        debug!("look for {:?}", key);
    ...
    pub fn get_ref(&self) -> &MgoPeers {
        &self.nodes
    }

    /// Get a mutable reference to the inner service
    pub fn get_mut(&mut self) -> &mut MgoPeers {
        &mut self.nodes
    }
...
/// extract will get the network pubkey bytes from a MgoValidatorSummary type.  This type comes from a
...
fn extract(summary: MgoSystemStateSummary) -> impl Iterator<Item = (Ed25519PublicKey, MgoPeer)> {
    summary.active_validators.into_iter().filter_map(|vm| {
        match Ed25519PublicKey::from_bytes(&vm.network_pubkey_bytes) {
            ...
            Err(error) => {
                error!(
                    "unable to decode public key for name: {:?} mgo_address: {:?} error: {error}",
                    vm.name, vm.mgo_address
                );
...
mod tests {
    ...
    use serde::Serialize;
    use mgo_types::mgo_system_state::mgo_system_state_summary::{
        MgoSystemStateSummary, MgoValidatorSummary,
    };

    /// creates a test that binds our proxy use case to the structure in mgo_getLatestMgoSystemState
    ...
    #[test]
    fn depend_on_mgo_mgo_system_state_summary() {
        let CertKeyPair(_, client_pub_key) = generate_self_cert("mgo".into());
        ...
        // some of those.
        let depends_on = MgoSystemStateSummary {
            active_validators: vec![MgoValidatorSummary {
        ...
        let r = serde_json::to_string(&ResponseBody { result: depends_on })
            .expect("expected to serialize ResponseBody{MgoSystemStateSummary}");

        let deserialized = serde_json::from_str::<ResponseBody>(&r)
            .expect("expected to deserialize ResponseBody{MgoSystemStateSummary}");
